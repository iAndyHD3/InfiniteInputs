...
class $modify(MyLayer, UILayer) {
    ...
    struct Fields {
        ...
        boost::unordered_flat_map<CCTouch*, boost::unordered_flat_set<ClickActionData*>, TouchHasher, TouchEquality>
                claimedTouches;
    ...
    // reason for this:
    //  If a touch starts on a non-UI object and the camera moves, the object's location will be updated according to
    //  the new camera position, which can cause it to no longer intersect with the originally touched object. By
    //  checking for this and updating the touch's position accordingly, we can ensure that the touch continues to
    //  interact with the intended object even if the camera moves.
    void updateNonUILayerTouches(float) {
        ...
        Log.i("UILayer", "DIAG ENTER: claimedTouches={}, clickActions(size={}, cap={}, data={:x}), dataAtInsert={:x}{}",
              mapSize, clickVec ? clickVec->size() : -1, clickVec ? clickVec->capacity() : -1, curDataPtr,
              m_fields->clickActionsDataAtInsert, reallocd ? " *** REALLOCATED ***" : "");
        ...
        for (auto& [touch, actionDataSet] : m_fields->claimedTouches) {
            ...
            for (auto* data : actionDataSet) {
                ...
                if (!data) {
                    ...
                    Log.e("UILayer",
                          "DIAG CRASH: collblock is NULL! data={:x}, taken={}, calledEnter={}, calledExit={}",
                          reinterpret_cast<uintptr_t>(data), data->taken, data->calledEnter, data->calledExit);
                    if (clickVec) {
                        ptrdiff_t idx = data - clickVec->data();
                        Log.e("UILayer",
                              "DIAG CRASH: clickActions size={}, cap={}, data={:x}, computed_idx={}, dataAtInsert={:x}",
                              clickVec->size(), clickVec->capacity(), reinterpret_cast<uintptr_t>(clickVec->data()),
                              idx, m_fields->clickActionsDataAtInsert);
                    }
                }
                if (!data->collblock->m_isUIObject) {
                    hasNonUI = true;
                    break;
                }
            }
    ...
    bool ccTouchBegan(CCTouch* touch, CCEvent* event) {
        ...
        for (ClickActionData& actionData : gf->clickActions) {
            ...
            if (!actionData.action.jump && touchInside && actionData.action.ignoreInput) {
                Log.i("UILayer",
                      "Touch id {} began inside click action with ignoreInput=true, spawning down group and blocking "
                      "touch",
                      touch->getID());
        ...
        if (callOriginal && !UILayer::ccTouchBegan(touch, event))
            return false;
    ...
    void touchMoved(CCTouch* touch) {
        ...
        for (auto* current : actionDataSet) {
            ...
            if (isInside && !current->calledEnter) {
                ...
                if (current->action.jump) {
                    layer->m_queuedButtons.push_back(
                            PlayerButtonCommand{
